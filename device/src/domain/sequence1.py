...
def run_maf_sampling_sequence(
    *,
    ...
    try:
        ...
        _run_step("Step 24: Close PID valve", pid_controller.force_close, wait_after=0.0)
        _run_step("Step 11: Enable peristaltic pump", _enable_peristaltic)
        _run_step("Step 12: PID enable", lambda: pid_controller.set_enabled(True))
        _run_step("Step 13: Start reading flow sensor / volume", start_flow_meter)
        _run_step(
            f"Step 14: Run pump until total volume >= {target_volume_ml:.1f} mL",
            _volume_loop,
        )
        _run_step("Step 15: Stop flow meter readings", stop_flow_meter, wait_after=0.5)
        _run_step(
            f"Step 16: Hold for {post_volume_wait_s:.1f}s after reaching target volume",
            wait_after=post_volume_wait_s,
        )
        _run_step(
            "Step 17: Switch peristaltic to HIGH speed",
            lambda: motor_pump.set_speed_checked(False),
            wait_after=30.0,
        )
        _run_step("Step 23: Disable PID valve control", lambda: pid_controller.set_enabled(False))
        _run_step("Step 24: Close PID valve", pid_controller.force_close, wait_after=10.0)
        _run_step("Step 24: Home PID valve", _home_pid, wait_after=15.0)
        _run_step("Step 24: Close PID valve", pid_controller.force_close, wait_after=10.0)
        _run_step("Step 18: Stopping peristaltic pump", _disable_peristaltic)
        _run_step("Step 19: Enable temperature controller", enable_temp_controller)
        _run_step("Step 20: Valve 1 CLOSED (Relay 1 OFF)", lambda: relays.off(1))
        _run_step("Step 21: Syringe move to 0.1 mL", lambda: _syringe_move(0.1, 1.0))
        _run_step("Step 22: Valve 1 OPEN (Relay 1 ON)", lambda: relays.on(1))
        _run_step("Step 23: Syringe move to 1.6 mL", lambda: _syringe_move(1.5, 1.0))
        _run_step("Step 25: Valve 2 OPEN (Relay 2 ON)", lambda: relays.on(2))
        _run_step("Step 26: Valve 1 CLOSED (Relay 1 OFF)", lambda: relays.off(1))
        _run_step("Step 27: Valve 3 CLOSED (Relay 3 ON)", lambda: relays.on(3))
        _run_step("Step 28: Syringe move to 0 mL", lambda: _syringe_move(0, 1.0))
        _run_step("Step 29: Valve 2 CLOSED (Relay 2 OFF)", lambda: relays.off(2))
        _run_step("Step 30: Syringe move to 2 mL", lambda: _syringe_move(2, 1.0))
        _run_step("Step 31: Wait to reach temperature (READY)", wait_for_temp_ready)
        _run_step("Step 32: Wait for MAF heating", wait_for_maf_heating)
        _run_step("Step 33: Disable temperature controller", disable_temp_controller)
        _run_step("Step 34: Enable fan", lambda: relays.on(8))
        _run_step("Step 35: Valve 4 OPEN (Relay 4 OFF)", lambda: relays.off(4))
        _run_step("Step 36: Valve 1 CLOSE (Relay 1 OFF)", lambda: relays.off(1))
        _run_step("Step 37: Valve 2 OPEN (Relay 2 ON)", lambda: relays.on(2))
        _run_step(
            "Step 38: Syringe move to 0 mL", lambda: _syringe_move(0, 2.0), wait_after=5.0
        )
        _run_step("Step 39: Vertical axis to TOP (open plate)", move_vertical_open_plate)
        _run_step("Step 41: Eject MAF (Relay 7 ON)", lambda: relays.on(7))
        _run_step("Step 42: Hold after ejecting MAF", wait_after=1.0)
        _run_step("Step 43: Relay 7 OFF", lambda: relays.off(7))
        _run_step("Step 44: Disable Fan (Relay 8 OFF)", lambda: relays.off(8))
        _run_step("Step 45: Home PID valve", _home_pid, wait_after=15.0)
        _run_step("Step 46: Relay 2 OFF", lambda: relays.off(2))
        _run_step("Step 47: Relay 3 OFF", lambda: relays.off(3))
        _run_step("Step 48: Horizontal axis to HOME (0 position)", move_horizontal_to_home)
    except SequenceAbort:
